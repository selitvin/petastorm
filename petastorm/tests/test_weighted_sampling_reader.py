...
def test_real_reader(synthetic_dataset):
    readers = [Reader(synthetic_dataset.url, predicate=in_lambda(['id'], lambda i: i % 2 == 0), num_epochs=None),
               Reader(synthetic_dataset.url, predicate=in_lambda(['id'], lambda i: i % 2 == 1), num_epochs=None)]
    results = [0, 0]
    num_of_reads = 300
    with WeightedSamplingReader(readers, [0.5, 0.5]) as mixer:
        # Use iteratable interface of WeightedSamplingReader in this case
        for i, next_sample in enumerate(mixer):
            next_id = next_sample.id % 2
            results[next_id] += 1
            if i >= num_of_reads:
                break
